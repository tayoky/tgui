...
static void tgui_focusable_state_changed(tgui_focusable_t *focusable) {
	if (tgui_widget_get_state(TGUI_WIDGET_CAST(focusable), TGUI_STATE_DISABLED)
			&& tgui_widget_get_state(TGUI_WIDGET_CAST(focusable), TGUI_STATE_FOCUSED)) {
		// we cannot be focused anymore
		tgui_surface_t *surface = tgui_widget_get_surface(TGUI_WIDGET_CAST(focusable));
...
static int tgui_focusable_constructor(void *object) {
	...
	tgui_widget_connect_signal(TGUI_WIDGET_CAST(focusable), "state-changed", TCALLBACK_CAST(tgui_focusable_state_changed), NULL);
	return 0;
}
